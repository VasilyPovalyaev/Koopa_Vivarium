...
/**
 * WiFi application event group handle
*/
static EventGroupHandle_t wifi_app_event_group;
const int WIFI_CONNECTING_USING_SAVED_CREDS_BIT = BIT0;
const int WIFI_CONNECTING_FROM_HTTP_SERVER_BIT = BIT1;
const int WIFI_APP_USER_REQUESTED_DISCONNECT_BIT = BIT2;
const int WIFI_APP_STA_CONNECTED_GOT_IP_BIT = BIT3;

//Queue handle used to manipulate the main queue of events
static QueueHandle_t wifi_app_queue_handle;

//netif objects fo the station and access point
esp_netif_t* esp_netif_sta = NULL;
esp_netif_t* esp_netif_ap = NULL;

/**
 * WiFi application event handler
 * @param arg data, aside from event data, that is oassed the the handler when it is called
    ...
    if (event_base == WIFI_EVENT)
    {
        switch (event_id)
        {
            ...
            case WIFI_EVENT_STA_DISCONNECTED:
                ESP_LOGI(TAG,"WIFI_EVENT_STA_DISCONNECT");
            ...
            case WIFI_EVENT_STA_CONNECTED:
                ESP_LOGI(TAG,"WIFI_EVENT_STA_CONNECT");
                break;
...
/**
 * Initializes the WiFi application event and IP events
 */
...
/**
 * Initialize the TCP stack and default wifi config
 */
...
static void wifi_app_default_wifi_init(void)
{
    ...
    //defualt wifi config - must be in this order
...
/**
 * Configure AP settings and assigns static IP to the soft AP
*/
static void wifi_app_soft_ap_config(void)
{
    //Soft AP - WiFi access point config - 
    ...
    inet_pton(AF_INET, WIFI_AP_IP, &ap_ip_info.ip);                         //> assign static ip, gateway and mask
    ...
    ESP_ERROR_CHECK(esp_netif_dhcps_start(esp_netif_ap));                   //> starts the AP DHCP server

    ESP_ERROR_CHECK(esp_wifi_set_mode(WIFI_MODE_APSTA));                    //> setting the mode as AP/STA
    ESP_ERROR_CHECK(esp_wifi_set_config(ESP_IF_WIFI_AP, &ap_config));       //> set config
    ESP_ERROR_CHECK(esp_wifi_set_bandwidth(WIFI_IF_AP, WIFI_AP_BANDWIDTH)); //> default 20MHz
    ...
    for(;;)
    {
        if (xQueueReceive(wifi_app_queue_handle, &msg, portMAX_DELAY))
        {
            ...
            case WIFI_APP_MSG_CONNECTING_FROM_HTTP_SERVER:
                ...
                xEventGroupSetBits(wifi_app_event_group, WIFI_CONNECTING_FROM_HTTP_SERVER_BIT);
                ...
                //let http server know about the connection attempt
            ...
            case WIFI_APP_MSG_STA_CONNECTED_GOT_IP:
                ...
                if(eventBits & WIFI_CONNECTING_FROM_HTTP_SERVER_BIT)
                {
            ...
            case WIFI_APP_MSG_STA_DISCONNECTED:
                ...
                if(eventBits & WIFI_APP_MSG_STA_CONNECTED_GOT_IP)
                {
                    xEventGroupClearBits(wifi_app_event_group, WIFI_APP_MSG_STA_CONNECTED_GOT_IP);
                }
                break;
